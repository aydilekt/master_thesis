...
trucks = trucks[trucks.distances > 500]
...
for t in range(len(truck_list)):
    ...
    for i in range(0, 5):
        print(i)
        type = "-hs-random-S1.3-" + str(i) + ".xlsx"
        trucks_sampled = trucks.sample(truck_sample)
        rest_sampled = rest_places.sample(rest_sample)
        trucks_sampled.to_excel("sample_truck-" + str(truck_sample) + str(type), sheet_name="truck", index=False)
